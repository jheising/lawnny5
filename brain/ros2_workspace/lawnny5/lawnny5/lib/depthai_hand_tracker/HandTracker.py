...
from .mediapipe_utils import find_isp_scale_params, find_isp_scale_params, HandRegion, rotated_rect_to_points, recognize_gesture, generate_handtracker_anchors
...
class HandTracker:
    ...
    def __init__(self, input_src=None,
        ...
        if input_src == None or input_src == "rgb" or input_src == "rgb_laconic":
            ...
            if self.crop:
                self.frame_size, self.scale_nd = find_isp_scale_params(internal_frame_height, self.resolution)
                self.img_h = self.img_w = self.frame_size
                self.pad_w = self.pad_h = 0
                self.crop_w = (int(round(self.resolution[0] * self.scale_nd[0] / self.scale_nd[1])) - self.img_w) // 2
            else:
                width, self.scale_nd = find_isp_scale_params(internal_frame_height * self.resolution[0] / self.resolution[1], self.resolution, is_height=False)
        ...
        self.anchors = generate_handtracker_anchors(self.pd_input_length, self.pd_input_length)
        ...
        if use_handedness_average:
            ...
            self.handedness_avg = [HandednessAverage() for i in range(self.max_hands)]
    ...
    def pd_postprocess(self, inference):
        ...
        if not self.solo:
            hands = non_max_suppression(hands, self.pd_nms_thresh)[:self.max_hands]
        if self.use_lm:
    ...
    def next_frame(self):
        ...
        if self.use_lm:
            ...
            if len(self.hands) == 2: 
                dist_rect_centers = distance(np.array((self.hands[0].rect_x_center_a, self.hands[0].rect_y_center_a)), np.array((self.hands[1].rect_x_center_a, self.hands[1].rect_y_center_a)))
